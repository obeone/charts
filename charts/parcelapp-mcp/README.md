...
- **Container image:** `harbor.obeone.cloud/private/parcelapp-mcp` (private registry)
- **Chart source:** [`charts/parcelapp-mcp`](https://github.com/obeone/charts/tree/main/charts/parcelapp-mcp)

## Prerequisites

- Helm 3
- A Kubernetes cluster matching the chart's `kubeVersion` constraint (see [`Chart.yaml`](./Chart.yaml))- A Parcel subscription and an API key (Parcel app → Settings → Parcel API)
- Credentials for the registry hosting the image, referenced through `defaultPodOptions.imagePullSecrets`
...
| controllers.main.containers.main.image.repository | string | `"harbor.obeone.cloud/private/parcelapp-mcp"` | Container image. Hosted on a private registry; override `repository` (and add `imagePullSecrets` above) to pull your own build. |
| controllers.main.containers.main.image.tag | string | `"private-version"` | Image tag. This is a mutable tag that is rebuilt in place, which is why it does not track the chart's appVersion. Pin an immutable digest or tag here if you publish one. |
